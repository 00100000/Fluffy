...
exports.run = async (client, message, args) => {
    ...
    const promoteEmbed = new MessageEmbed()
        ...
        .addField("Promoted To", message.guild.roles.get(roleHierarchy[rolePosition + 1]).name, false)
        .addField("Server", message.guild.name + `(${message.guild.id})`, false)
        .setColor(embedColor)
        .setTimestamp();

    user.send(`You've been promoted by ${message.author.tag}, in ${message.guild.name} to ${message.guild.roles.get(roleHierarchy[rolePosition + 1]).name}`)
        .catch(() => {
