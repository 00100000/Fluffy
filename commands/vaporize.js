...
exports.run = async (client, message, args) => {
	...
	if (!user) {
		...
		message.guild.members.ban(user, { days: 7, reason: `${message.author.tag}: ${args.slice(1).join(" ")}` })
			.then(() => {
				message.channel.send(`${successEmoji} ${user} has been vaporized.`);
			}).catch(e => {
	...
	if (message.guild.member(user)) {
		if (!message.guild.member(user).bannable) return message.channel.send("This user is too powerful to be vaporized!");
	...
	user.send(`You've been vaporized by ${message.author.tag}, in ${message.guild.name} for ${reason}.`)
		.catch(() => {
